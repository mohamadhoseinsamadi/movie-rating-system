...
-- ===============================
-- DATA LOAD ONLY
-- Assumes the final schema is already created by Alembic.
-- Loads 1000 REAL movies (titles, directors, cast, genres) from TMDB 5000 dataset.
-- Requires: tmdb_5000_movies.csv and tmdb_5000_credits.csv in the same directory as this script.
-- Recommended run (interactive psql):
-- psql -U movieuser -d moviedb -h localhost -f seeddb.sql
-- ===============================

BEGIN;

-- --------------------------- 1. Cleanup Existing Data ---------------------------
-- Delete existing data to ensure a clean run
...
-- NOTE: Use psql interactive for \copy, or replace with COPY if server can access file
-- Adjust path as needed
COPY tmdb_movies_raw (budget,genres,homepage,id,keywords,original_language,original_title,overview,popularity,production_companies,production_countries,release_date,revenue,runtime,spoken_languages,status,tagline,title,vote_average,vote_count) 
FROM '/tmp/tmdb_5000_movies.csv' CSV HEADER;

COPY tmdb_credits_raw (movie_id,title,"cast",crew) 
FROM '/tmp/tmdb_5000_credits.csv' CSV HEADER;
...
SELECT DISTINCT
    ...
    'Imported from TMDB credits as Director'
...
-- Criteria: highest vote_count & must have a director
...
WITH joined AS (
    SELECT
        ...
        (
            ...
            WHERE c2->>'job' = 'Director' AND c2->>'name' IS NOT NULL
            LIMIT 1
        ) AS director_name
    FROM tmdb_movies_raw m
    JOIN tmdb_credits_raw cr ON cr.movie_id = m.id
),
filtered AS (
    SELECT *,
        row_number() OVER (
...
SELECT tmdb_id, title, genres, release_date, "cast", crew, director_name
FROM filtered
WHERE rn <= 1000;
...
SELECT
    s.title,
    d.id AS director_id,
    COALESCE(NULLIF(split_part(s.release_date, '-', 1), '')::INT, 2000) AS release_year,
    (
...
JOIN directors d ON d.name = s.director_name
ORDER BY s.tmdb_id;

-- --------------------------- 9. Insert Movie Genres ---------------------------
INSERT INTO movie_genres (movie_id, genre_id)
SELECT mv.id AS movie_id, g.id AS genre_id
FROM movies mv
...
JOIN genres g ON g.name = gj->>'name'
GROUP BY mv.id, g.id;
...
SELECT
    ...
    now() - (random() * INTERVAL '5 years') AS rated_at
FROM movies m,
